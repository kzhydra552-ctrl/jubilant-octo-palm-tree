...
function normalizePublicJobId(value) {
  if (value === null || value === undefined) return null;
  const candidate = String(value).trim();
  // Roblox public game.JobId é um UUID; hashes/token assinados não são destinos de teleporte.
...
function extractPublicJobId(raw) {
  const match = String(raw ?? '').match(/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/i);
  return normalizePublicJobId(match?.[0]);
}

function readEventLocation(candidates, raw) {
  let jobId = normalizePublicJobId(process.env.ROBLOX_JOB_ID || process.env.SERVER_JOB_ID || process.env.GAME_JOB_ID) || extractPublicJobId(raw);
  let placeId = normalizePlaceId(process.env.ROBLOX_PLACE_ID || process.env.GAME_PLACE_ID);
  for (const candidate of candidates) {
    if (!jobId) jobId = normalizePublicJobId(candidate.jobId ?? candidate.job_id ?? candidate.jobID ?? candidate.server ?? candidate.serverId ?? candidate.server_id ?? candidate.instanceId ?? candidate.instance_id ?? candidate.gameServerId);
    if (!placeId) placeId = normalizePlaceId(candidate.placeId ?? candidate.place_id ?? candidate.gameId ?? candidate.game_id ?? candidate.universePlaceId);
    if (jobId && placeId) break;
  }
  return { jobId: jobId || null, placeId: placeId || null };
}

function scanLocationDeep(value, depth = 0, result = { jobId: null, placeId: null }) {
  if (!value || depth > 8 || result.jobId && result.placeId) return result;
  if (Array.isArray(value)) {
    for (const item of value) scanLocationDeep(item, depth + 1, result);
    return result;
  }
  if (typeof value !== 'object') return result;
  for (const [key, nested] of Object.entries(value)) {
    const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
    if (!result.jobId && /(jobid|serverid|instanceid|gameinstance|gameserver)/.test(normalizedKey)) result.jobId = normalizePublicJobId(nested);
    if (!result.placeId && /(placeid|gameid|universeplace)/.test(normalizedKey)) result.placeId = normalizePlaceId(nested);
    scanLocationDeep(nested, depth + 1, result);
  }
  return result;
}
...
function parseEvent(raw) {
  let data = raw;
  const rawJobId = extractPublicJobId(raw);
  try {
  ...
  collect(data);
  const shallowLocation = readEventLocation(candidates, raw);
  const deepLocation = scanLocationDeep(data);
  const location = { jobId: shallowLocation.jobId || deepLocation.jobId, placeId: shallowLocation.placeId || deepLocation.placeId };
  ...
  if (pipeName && !pipeName.startsWith('{') && !pipeName.startsWith('[')) {
    ...
    if (classifyByName(pipeName).length) return { name: pipeName, value, count: parts[2] || '-', eventId: parts.slice(0, 4).join('|') || `${pipeName}|${Date.now()}`, jobId: location.jobId || rawJobId, placeId: location.placeId };
  }
