...
function classifyByName(name) {
  ...
  if (OG_NAMES.has(normalized)) return { tier: 'OG', url: WEBHOOK_OG, color: 0xffd600 };
  if (ULTRA_NAMES.has(normalized)) return { tier: 'ULTRA', url: WEBHOOK_ULTRA, color: 0xff1744 };
  if (HIG_NAMES.has(normalized)) return { tier: 'HIG', url: WEBHOOK_HIGH, color: 0xff9100 };
  if (MID_NAMES.has(normalized)) return { tier: 'MID', url: WEBHOOK_MID, color: 0x00c853 };
  return null;
}
...
async function routeEvent(event) {
  ...
  const match = classifyByName(event.name);
  if (!match) {
    log(`event ${event.name} ignored: name not in MID/HIG/ULTRA/OG filters`);
    return;
  }

  const jobs = [];
  if (WEBHOOK_JOBID) jobs.push(sendWebhook(WEBHOOK_JOBID, event, 'JOB ID', match.color));
  if (match.url) jobs.push(sendWebhook(match.url, event, match.tier, match.color));
  if (!jobs.length) return;
  const results = await Promise.allSettled(jobs);
  const failed = results.filter((result) => result.status === 'rejected');
  if (failed.length) log(`event ${event.name} (${match.tier}) sent with ${failed.length} webhook error(s)`);
  else log(`event ${event.name} (${match.tier}) ${formatValue(event.value)} forwarded`);
}
