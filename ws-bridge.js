...
function parseEvent(raw) {
  ...
  for (const candidate of candidates) {
    ...
    if (name) {
      return {
        ...
        jobId: normalizePublicJobId(candidate.jobId ?? candidate.job_id ?? candidate.server ?? candidate.serverId ?? candidate.instanceId),
  ...
  if (pipeName && !pipeName.startsWith('{') && !pipeName.startsWith('[')) {
    ...
    if (classifyByName(pipeName).length) return { name: pipeName, value, count: parts[2] || '-', eventId: parts.slice(0, 4).join('|') || `${pipeName}|${Date.now()}`, jobId: null, placeId: null };
  }
