...
const WS_URL_2 = env('WS_URL_2', 'wss://us.lasupremenotifier.com/ws/c-6lApxVzRLZhi');
...
const FILTER_VERSION = 'explicit-channel-filters-v5-dual-ws';
...
const connections = [
  ...
  { label: 'SUPREME', url: WS_URL_2, secret: SECRET_KEY_2, reconnectDelay: 2000, reconnectTimer: null, socket: null },
];
...
function connect(connection) {
  if (!connection.url) {
    log(`disabled: ${connection.label} WS_URL is not configured`);
    return;
  }
  if (!/^wss?:\/\/[^\s]+$/i.test(connection.url)) {
    log(`disabled: ${connection.label} WS_URL inválida`);
    return;
  }
  ...
  let currentSocket;
  try {
    currentSocket = new WebSocket(connection.url, headers ? { headers } : undefined);
  } catch (error) {
    log(`${connection.label} connection setup error: ${error.message}`);
    scheduleReconnect(connection);
    return;
  }
  connection.socket = currentSocket;
  const heartbeat = setInterval(() => {
    if (connection.socket === currentSocket && currentSocket.readyState === WebSocket.OPEN) currentSocket.ping();
  }, 20000);

  currentSocket.on('open', () => {
    connection.reconnectDelay = 2000;
    log(`${connection.label} connected`);
  });
  currentSocket.on('message', (message) => {
    const event = parseEvent(message.toString());
    if (event) routeEvent(event).catch((error) => log(`route error (${connection.label}): ${error.message}`));
  });
  ...
  currentSocket.on('close', (code, reason) => {
    ...
    log(`${connection.label} closed code=${code} reason=${reason.toString().slice(0, 120)}`);
    scheduleReconnect(connection);
  });
}
...
function startWsBridge() {
  ...
  if (!ENABLE_DEX_WS) {
    log('disabled: DEX WS connections are OFF; set ENABLE_DEX_WS=true only to re-enable');
    return;
  }
