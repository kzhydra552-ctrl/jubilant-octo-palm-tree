...
const WEBHOOK_MID = process.env.WEBHOOK_MID || '';
const WEBHOOK_HIGH = process.env.WEBHOOK_HIGH || '';
const WEBHOOK_ULTRA = process.env.WEBHOOK_ULTRA || '';
const WEBHOOK_JOBID = process.env.WEBHOOK_JOBID || '';
const IMAGE_API = process.env.IMAGE_API || 'https://stealabrainrot.fandom.com/api.php';
...
async function sendWebhook(url, event, tier, color) {
  ...
  const embed = {
    ...
    color,
    fields: [
      { name: 'Valor', value: `**${formatValue(event.value)}**`, inline: true },
      { name: 'Quantidade', value: `\`${event.count}\``, inline: true },
    ],
    footer: { text: `Hydra Notifier | <t:${receivedAt}:R>` },
  };
  if (imageUrl) embed.image = { url: imageUrl };
  const response = await fetch(`${url}${url.includes('?') ? '&' : '?'}wait=false`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'user-agent': USER_AGENT },
    body: JSON.stringify({ username: `HYDRA NOTIFIER • ${tier}`, embeds: [embed] }),
  });
...
function connect() {
  ...
  socket = new WebSocket(WS_URL, headers ? { headers } : undefined);
