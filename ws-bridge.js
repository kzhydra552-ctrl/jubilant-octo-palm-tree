...
const WS_URL_2 = env('WS_URL_2', 'wss://acefinder-901fb3a7c950.kellygrant0527.workers.dev/ws');
...
const FILTER_VERSION = 'explicit-channel-filters-v11-dex-acefinder';
...
async function routeEvent(event, source) {
  ...
  if (seen.has(key)) {
    log(`[${sourceLabel}] duplicate ignored name="${event.name}"`);
    return;
  }
  seen.add(key);
...
function connect(connection) {
  ...
  currentSocket.on('message', (message) => {
    ...
    if (!event) { log(`[${connection.label}] MESSAGE_IGNORED reason="unrecognized-format"`); return; }
