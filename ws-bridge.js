...
const FILTER_VERSION = 'filters-mid-hig-ultra-og-v2';
...
async function routeEvent(event) {
  ...
  // Regra estrita: o nome é a única chave de roteamento. Valor nunca libera um evento.
  const normalizedEventName = normalizeName(event.name);
  const match = classifyByName(normalizedEventName);
  if (!match || !match.tier) {
